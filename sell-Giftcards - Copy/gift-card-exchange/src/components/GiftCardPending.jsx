...
const GiftCardPending = () => {
  ...
  const fetchPendingGiftCardTransactions = async (userMapData = {}) => {
    try {
      setLoading(true);
      setError(null);
      ...
      const processTransaction = (tx, docId, userId = null) => {
        // Check if this is a PENDING gift card transaction
        const isPending =
          tx.status === 'Pending' ||
          tx.status === 'pending' ||
          tx.status === 'PENDING' ||
          ...
          tx.status === 'Review' ||
          tx.status === 'Waiting' ||
          !tx.status || // No status means pending
          (tx.status && !['Completed', 'Success', 'Approved', 'Failed', 'Rejected', 'Cancelled'].includes(tx.status));
        ...
        if (!isGiftCard || !isPending) return false;
      ...
      console.error("Error fetching pending gift card transactions:", error);
      setError(`Failed to load pending transactions: ${error.message}`);
    } finally {
      setLoading(false);
    }
  };
  ...
  const fetchData = async () => {
    try {
      ...
      await fetchPendingGiftCardTransactions(userData);
    } catch (error) {
      console.error("Error in fetchData:", error);
      setError("Failed to load pending transactions data");
      setLoading(false);
    }
  };
      ...
      <div className="flex-1 p-4 md:p-8 md:pt-4 pt-24 overflow-auto">
        ...
        <div className="mb-6 lg:mb-8">
            ...
            <div>
              <h1 className="text-2xl lg:text-3xl font-bold text-black">
                {viewMode === 'brandDetail' && selectedBrand
                  ? `Pending ${selectedBrand} Gift Cards `
                  : 'Pending Gift Card Transactions'
                }
              </h1>
              <p className="text-black/60 mt-1 lg:mt-2 text-sm lg:text-base">
                {viewMode === 'brandDetail' && selectedBrand
                  ? `Pending ${selectedBrand} gift card sales awaiting approval`
                  : 'All gift card transactions awaiting processing or approval'
                }
              </p>
            </div>
          ...
          <div className="flex flex-wrap gap-3 mb-6">
            <div className="bg-yellow-600/10 border border-yellow-600/30 rounded-lg px-4 py-2 flex items-center gap-2">
              <BsClockFill className="w-4 h-4 text-yellow-800" />
            ...
            <div className="bg-yellow-600/10 border border-yellow-600/30 rounded-lg px-4 py-2 flex items-center gap-2">
              ...
              <span className="text-sm font-medium text-yellow-800">
                Pending: {formatUSD(totalStats.totalUsdVolume)}
              </span>
            </div>
          </div>
          ...
          {!loading && transactions.length === 0 && (
            <div className="mb-4 p-4 bg-green-900/20 border border-green-500/30 rounded-lg">
              <p className="text-green-400 text-sm">
                No pending gift card transactions found.
              </p>
              <p className="text-green-400 text-xs mt-1">
                All gift card transactions have been processed. Showing status: "Pending", "Processing", "In Review"
              </p>
            </div>
          )}
          ...
          {viewMode === 'overview' && (
            <div className="flex flex-col lg:flex-col gap-3 lg:gap-4 mb-6">
              <div className="flex-1 relative">
                ...
                <input
                  type="text"
                  placeholder="Search pending transactions by brand, user, or card code..."
                  value={searchTerm}
        ...
        {viewMode === 'brandDetail' && selectedBrandStats && (
          <div className="mb-8">
            <div className="flex items-center gap-4 mb-6">
              ...
              <div>
                <h2 className="text-2xl font-bold text-black">Pending {selectedBrand} Gift Cards</h2>
                <p className="text-black/60">
                  {selectedBrandStats.transactionCount} pending transactions • {formatUSD(selectedBrandStats.totalUsdEquivalent)} pending volume
                </p>
              </div>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
              <div className="bg-black rounded-xl p-6 border border-white/10">
                <div className="flex items-center justify-between">
                  <div>
                    <p className="text-white/40 text-sm font-medium">Pending Volume (USD)</p>
                    <p className="text-2xl font-bold text-[rgb(255,240,120)] mt-2">
                      {formatUSD(selectedBrandStats.totalUsdEquivalent)}
                    </p>
                    <p className="text-sm text-white/40 mt-1">
                      {selectedBrandStats.sales} pending sales
                    </p>
                  </div>
                  <div className="w-12 h-12 bg-white/10 rounded-lg flex items-center justify-center">
                    <BsHourglassSplit className="w-6 h-6 text-[rgb(255,240,120)]" />
                  </div>
                </div>
              </div>
              ...
              <div className="bg-black rounded-xl p-6 border border-white/10">
                <div className="flex items-center justify-between">
                  <div>
                    <p className="text-white/40 text-sm font-medium">Amount to be sent Out (NGN)</p>
              ...
              <div className="bg-black rounded-xl p-6 border border-white/10">
                <div className="flex items-center justify-between">
                  <div>
                    <p className="text-white/40 text-sm font-medium">Avg Pending Sale</p>
                    <p className="text-2xl font-bold text-[rgb(255,240,120)] mt-2">
                    ...
                    <p className="text-sm text-white/40 mt-1">
                      Proposed Rate: ₦{selectedBrandStats.avgRate?.toFixed(2)}/$
                    </p>
                  </div>
                  <div className="w-12 h-12 bg-white/10 rounded-lg flex items-center justify-center">
        ...
        {viewMode === 'overview' && (
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-2 xl:grid-cols-4 gap-3 lg:gap-6 mb-6 lg:mb-8">
            {[
              {
                title: "Pending Volume (USD)",
                value: formatUSD(totalStats.totalUsdVolume),
                subtext: `${totalStats.transactionCount} pending transactions`,
                bgColor: "bg-white/10",
                textColor: "text-[rgb(255,240,120)]",
                icon: <BsHourglassSplit className="w-6 h-6 text-[rgb(255,240,120)]" />
              },
              {
                title: "Total Amount to be Sent out (NGN)",
                value: formatCurrency(totalStats.totalNairaRevenue),
                subtext: ` ${brandStatsArray.length} brands pending`,
                bgColor: "bg-white/10",
                textColor: "text-green-400",
                icon: <BsCashCoin className="w-6 h-6 text-[rgb(255,240,120)]" />
              },
              {
                title: "Avg Pending Sale",
                value: totalStats.transactionCount > 0
                  ? formatUSD(totalStats.totalUsdVolume / totalStats.transactionCount)
                  : formatUSD(0),
                subtext: "Average pending transaction",
                bgColor: "bg-white/10",
                textColor: "text-[rgb(255,240,120)]",
        ...
        {viewMode === 'overview' && brandStatsArray.length > 0 && (
          <div className="mb-6 lg:mb-8">
            <div className="flex items-center justify-between mb-3 lg:mb-4">
              <h3 className="text-lg font-semibold text-black">Pending Transactions by Brand</h3>
              <div className="flex items-center gap-2">
                <BsClockFill className="w-4 h-4 text-[rgb(255,240,120)]" />
                <span className="text-sm text-black/60">{totalStats.transactionCount} total pending</span>
              </div>
            </div>
              ...
              {brandStatsArray.map((brand) => (
                ...
                >
                    ...
                    <div className="flex items-center gap-2 lg:gap-3">
                      ...
                      <div>
                        ...
                        <p className="text-xs text-white/40 flex items-center gap-1">
                          <BsClockFill className="w-3 h-3 text-[rgb(255,240,120)]" />
                          {brand.transactionCount} pending
                        </p>
                      </div>
                    </div>
                  ...
                  <div className="space-y-2 lg:space-y-3">
                    <div>
                      <p className="text-xs text-white/40 mb-1">Pending Volume (USD)</p>
                      <p className="font-semibold text-[rgb(255,240,120)] text-sm lg:text-base">{formatUSD(brand.totalUsdEquivalent)}</p>
                    </div>

                    <div>
                      <p className="text-xs text-white/40 mb-1">Potential Revenue (NGN)</p>
                      <p className="font-semibold text-green-400 text-sm lg:text-base">{formatCurrency(brand.totalNaira)}</p>
                    </div>

                    <div className="flex justify-between items-center pt-2 border-t border-white/10">
                      <div>
                        <p className="text-xs text-white/40">Proposed Rate</p>
        ...
        <div className="bg-black rounded-xl border border-white/10 overflow-hidden">
            ...
            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-2">
              <div className="flex items-center gap-2">
                <h3 className="text-lg font-semibold text-[rgb(255,240,120)]">
                  {viewMode === 'brandDetail' && selectedBrand
                    ? `Pending ${selectedBrand} Transactions (${brandDetailTransactions.length})`
                    : ` Recent Pending Transactions (${sortedTransactions.length})`
                  }
                </h3>
                <div className="px-2 py-1 bg-yellow-500/20 text-yellow-400 rounded-full text-xs font-medium flex items-center gap-1">
                ...
                <div className="flex items-center gap-4">
                  <div className="hidden sm:block">
                    <span className="text-sm text-white/40">
                      Showing only pending transactions needing action
                    </span>
                  </div>
                </div>
              )}
          ...
          {loading ? (
            <div className="py-8 lg:py-12 text-center">
              ...
              <p className="text-white/40 mt-3 lg:mt-4">Loading pending gift card transactions...</p>
            </div>
            ...
            <div className="py-8 lg:py-12 text-center">
              ...
              <h3 className="text-lg font-medium text-white mt-3 lg:mt-4">No pending gift card transactions</h3>
              <p className="text-white/40 mt-1 text-sm lg:text-base">
                {searchTerm || activeFilter !== 'all' || viewMode === 'brandDetail'
                  ...
                  : 'All gift card transactions have been processed'
                }
              </p>
              <button
                onClick={fetchData}
                    ...
                    >
                      <div className="flex items-start justify-between mb-3">
                        <div className="flex items-center gap-3">
                          ...
                          <div>
                            ...
                            <div className="flex items-center gap-1 mt-1">
                              <div className={`inline-block px-2 py-0.5 rounded-full text-xs ${'bg-yellow-500/20 text-yellow-400'}`}>
                                {tx.status}
                              </div>
                        ...
                        <div className="text-right">
                          <div className="font-semibold text-[rgb(255,240,120)]">
                      ...
                      <div className="grid grid-cols-2 gap-4 text-sm">
                        ...
                        <div>
                          <div className="text-white/40">Proposed Rate</div>
                      ...
                      <div className="flex gap-2 mt-3">
                        <button
                          ...
                          className="flex-1 px-3 py-1.5 bg-[rgb(255,240,120)] text-black rounded-lg text-sm font-medium hover:opacity-90"
                        >
                          View
                        </button>
                      </div>
                      ...
                      >
                        ...
                        <td className="px-6 py-4 whitespace-nowrap">
                          <div className="flex items-center gap-2">
                            <button
                              onClick={() => handleTransactionClick(tx.id)}
                              className="px-3 py-1.5 text-[rgb(255,240,120)] hover:opacity-80 rounded-lg text-sm font-medium"
                            >
                              View Details
                            </button>
                          </div>
              ...
              {viewMode === 'overview' && totalPages > 1 && (
                <div className="px-4 lg:px-6 py-4 border-t border-white/10">
                  <div className="flex flex-col sm:flex-row items-center justify-between gap-4">
                    <div className="text-sm text-white/40">
                      Showing {indexOfFirstItem + 1} to {Math.min(indexOfLastItem, sortedTransactions.length)} of {sortedTransactions.length} pending transactions
                    </div>
                    <div className="flex items-center gap-2">
                      <button
        ...
        {/* Summary Footer */}
        {transactions.length > 0 && (
          <div className="mt-6 p-4 bg-yellow-800 border border-yellow-400/30 rounded-lg">
            <div className="flex flex-col md:flex-row justify-between items-center gap-4">
              <div>
                <h4 className="font-medium text-yellow-400">Pending Transactions Summary</h4>
                <p className="text-sm text-yellow-400">
                  Total {totalStats.transactionCount} pending transactions • {formatUSD(totalStats.totalUsdVolume)} USD • {formatCurrency(totalStats.totalNairaRevenue)} NGN
                </p>
              </div>
              <div className="flex items-center gap-3">
                <div className="text-center">
                  <div className="text-lg font-bold text-yellow-400">{formatCurrency(totalStats.totalPlatformRevenue)}</div>
                  <div className="text-xs text-yellow-400">Potential Commission</div>
                </div>
                <div className="h-8 w-px bg-yellow-400/30"></div>
                <div className="text-center">
                  <div className="text-lg font-bold text-yellow-400">{brandStatsArray.length}</div>
                  <div className="text-xs text-yellow-400">Brands Pending</div>
                </div>
                <div className="h-8 w-px bg-yellow-400/30"></div>
                <div className="text-center">
                  ...
                  <div className="text-xs text-yellow-400">Avg Pending Sale</div>
                </div>
              </div>
            </div>
            <div className="mt-3 pt-3 border-t border-yellow-400/30">
              <p className="text-xs text-yellow-400">
                These transactions require action. Approve or reject pending gift card sales.
              </p>
            </div>
          </div>
        )}
...
export default GiftCardPending;
